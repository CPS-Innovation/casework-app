...
import { usePCD, usePCDList } from '../hooks';
import { APP_ROUTES_MAP } from '../routes';
...
export const PcdRequestPage = () => {
  ...
  const { pcdRequestRoute } = APP_ROUTES_MAP;
  const { data: pcdListData, isLoading: isPcdListLoading } = usePCDList(caseInfo?.id);
  ...
  return (
    ...
    <div className="govuk-main-wrapper" style={{ whiteSpace: 'pre-wrap' }}>
      {navLinks?.length === 0 && <p className="govuk-body">There are no PCD Requests to show.</p>}
                ...
                {pcdDetailsData?.caseOutline.map(({ heading, textWithCmsMarkup }, index) => (
                  <Fragment key={index}>
                    ...
                    <p
                      ...
                      dangerouslySetInnerHTML={{ __html: cleanString(textWithCmsMarkup) }}
                    />
                  </Fragment>
                ))}
